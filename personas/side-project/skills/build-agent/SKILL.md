...
2. **The trigger.** "When should it run?
   - On-demand (you ask it: 'plan this week's posts')
   - Scheduled (every Sunday evening)
   - Reactive (when something happens)"
...
> *"Read-only — it can look at your files but can't change them. Read + create — can also save new files (good for an agent that writes drafts to a folder). Full access — can also edit and delete; only pick this if you fully trust it not to mess up."*
...
Wait for their answer. Common worst cases for side projects:
- Posting half-baked content under your name
- Sending a draft email before it's ready
- Overwriting a draft you actually liked
- Spending real money on something you didn't approve
- Sharing notes that were supposed to be private

### 2. "What hard rules should this agent NEVER break?"

Help them list 3-5 specific rules. Examples:
- *Content:* "Never publish anything without me reviewing first." "Never tag specific people in posts."
- *Files:* "Never delete or modify existing drafts." "Only write to the `drafts/` folder."
- *Money:* "Never make any purchases." "Never sign me up for anything."
- *Voice/brand:* "Always sound like me, not like a corporate blog." "Never use AI-generated images without telling me."
- *Distribution:* "Don't post to social media — just write the draft for me to post myself."
...
- It doesn't remember things between runs unless you give it a notes file
- It can be wrong on real-time info, niche topics, or recent events
- It can hallucinate confidently — guardrails are how you catch this
- It can't truly know your audience the way you do — drafts will need editing
...
⚠️ Known limitations:
- [one or two — e.g., "doesn't know your audience the way you do", "drafts need a final pass"]
```
...
Save to `~/.claude/skills/[agent-name]/SKILL.md`. Use kebab-case based on purpose (e.g., `weekly-newsletter-drafter`, `launch-day-checklist`, `content-calendar-planner`).
...
If the agent needs to run on a schedule (like "every Sunday evening") or react to events (like "when I publish a post"):
...
- **Don't pretend the agent has access to tools you haven't actually configured.** If the user said "post to Twitter" but no Twitter integration is set up, name that gap clearly.
